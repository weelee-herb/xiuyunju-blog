...
- **2026-08-27 · 审计整改与体验优化**：题库死链临时收敛到《识药闯关》并标注维护中；Waline 公开接口增加边缘缓存；清除本草文章重复句号、补齐图片中文 alt；修复嵌套 `<main>` 与首页脚本位置；评论/登录/表情代码外置并延迟加载 Waline；优化列表标题层级、站点 meta、缓存策略和旧新文章互链。
