...
一座以「宋徽宗美学」为设计语言的个人博客：汝窑天青、宣纸底色、瘦金意趣标题、朱砂印章、竖排诗与瑞鹤线描。用于分享你用 **Codex / AI 创作**的内容，并带一个「访客留邮箱 → 自动收到回信」的订阅系统。
...
CAPTCHA_SECRET=随便一长串随机字符           # 评论验证码签名，建议单独设置
...
- **2026-08-27 · 审计整改与体验优化**：修复题库 Cloudflare Tunnel（本机 DoH 代理路径）并恢复 `herb-quiz.cc.cd` 正式入口；Waline 公开接口增加边缘缓存；清除本草文章重复句号、补齐图片中文 alt；修复嵌套 `<main>` 与首页脚本位置；评论/登录/表情代码外置并延迟加载 Waline；优化列表标题层级、站点 meta、缓存策略和旧新文章互链。
